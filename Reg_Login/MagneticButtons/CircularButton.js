class CircularButton {
  ...
  placeBy(placing) {
    switch (placing) {
      case "top":
        this.place(`${range(10, 90)} %`, "0 %");
        break;
      case "right":
        this.place("100 %", `${range(10, 90)} %`);
        break;
      case "bottom":
        this.place(`${range(10, 90)} %`, "100 %");
        break;
      case "left":
        this.place("0 %", `${range(10, 90)} %`);
        break;
    }
  }
...
class CircularButtonController {
  ...
  render() {
    ...
    buttons.forEach((button, index) => {
      ...
      button.replaceInfinitelyEvery(
        range(range(5, 11), range(16, 20)),
        this.pickPlacing(),
        () => this.getEmoji(this.emojiQueue)
      );
    });
  }
